@extends('layouts.app')
@section('title', $portfolio->document_name)

@section('content')

    {{-- Hero --}}
    <section class="bg-sbk-black py-28 relative overflow-hidden">
        {{-- Background Image & Overlay Dinamis --}}
        <div class="absolute inset-0 z-0">
            @if ($portfolio->image)
                <img src="{{ asset('storage/' . $portfolio->image) }}" class="w-full h-full object-cover"
                    alt="{{ $portfolio->document_name }}">
            @else
                {{-- Fallback kalau foto proyek tidak ada --}}
                <img src="{{ asset('images/construction-site.jpg') }}" class="w-full h-full object-cover" alt="">
            @endif
            {{-- Overlay Hitam (Agak gelap biar teks detail yang banyak tetap tajam) --}}
            <div class="absolute inset-0" style="background: rgba(0,0,0,0.8); z-index: 10;"></div>
        </div>

        {{-- Gradient Merah (Arah Top Right sesuai kode awalmu) --}}
        ...
        <div class="container-sbk relative z-20">
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                @if ($portfolio->document_category)
                    <span
            ...
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 bg-sbk-red rounded-lg flex items-center justify-center shadow-lg">
                    <span class="text-white font-black text-xs">
                        {{ strtoupper(substr($portfolio->client->name ?? 'K', 0, 1)) }}
                    </span>
                </div>
                <span class="text-gray-300 font-medium">{{ $portfolio->client->name ?? '-' }}</span>
            </div>
        </div>
    </section>
                    ...
                    @if ($portfolio->images->count())
                        ...
                        <div class="relative rounded-2xl overflow-hidden mb-4 cursor-pointer" x-data="{ lightbox: false, current: 0 }"
                            ...
                            <div class="relative h-[420px] rounded-2xl overflow-hidden group"
                                ...
                                <img src="{{ Storage::url($portfolio->images->first()->image_path) }}"
                                    alt="{{ $portfolio->images->first()->caption ?? $portfolio->document_name }}"
                                ...
                                @if ($portfolio->images->count() > 1)
                                    <div
                                        ...
                                        <div
                                            ...
                                            Lihat {{ $portfolio->images->count() }} Foto
                                        </div>
                                    </div>
                                @endif

                                {{-- Caption --}}
                                @if ($portfolio->images->first()->caption || $portfolio->images->first()->activity_type)
                                    <div
                                        class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6">
                                        @if ($portfolio->images->first()->activity_type)
                                            <span
                                                class="text-sbk-red text-xs font-bold uppercase tracking-widest block mb-1">
                                                {{ $portfolio->images->first()->activity_type }}
                                            </span>
                                        @endif
                                        @if ($portfolio->images->first()->caption)
                                            <p class="text-white font-semibold text-sm">
                                                {{ $portfolio->images->first()->caption }}</p>
                                        @endif
                                    </div>
                                @endif
                            </div>
                            ...
                            @if ($portfolio->images->count() > 1)
                                <div class="grid grid-cols-3 gap-3 mt-3">
                                    @foreach ($portfolio->images->skip(1)->take(3) as $i => $image)
                                        <div class="relative rounded-xl overflow-hidden h-28 cursor-pointer group"
                                            @click="lightbox = true; current = {{ $i + 1 }}">
                                            ...
                                            {{-- +N overlay kalau ada lebih dari 4 foto --}}
                                            @if ($loop->last && $portfolio->images->count() > 4)
                                                <div class="absolute inset-0 bg-black/60 flex items-center justify-center">
                                                    <span
                                                        class="text-white font-black text-xl">+{{ $portfolio->images->count() - 4 }}</span>
                                                </div>
                                            @endif
                                        </div>
                                    @endforeach
                        ...
                        text-white text-sm px-4 py-1.5 rounded-full font-medium">
                                    <span x-text="current + 1"></span> / {{ $portfolio->images->count() }}
                                </div>

                                {{-- Images --}}
                                <div class="relative max-w-5xl w-full">
                                    @foreach ($portfolio->images as $i => $image)
                                        <div x-show="current === {{ $i }}"
                                            ...
                                            @if ($image->caption || $image->activity_type)
                                                <div class="text-center mt-4">
                                                    @if ($image->activity_type)
                                                        <span
                                                            class="text-sbk-red text-xs font-bold uppercase tracking-widest">{{ $image->activity_type }}</span>
                                                    @endif
                                                    @if ($image->caption)
                                                        <p class="text-white text-sm mt-1">{{ $image->caption }}</p>
                                                    @endif
                                                </div>
                                            @endif
                                        </div>
                                    ...
                                    @if ($portfolio->images->count() > 1)
                                        <button
                                            @click="current = current === 0 ? {{ $portfolio->images->count() - 1 }} : current - 1"
                               ...
                               flex items-center justify-center text-white transition-all duration-200">
                                        ...
                                        <button
                                            @click="current = current === {{ $portfolio->images->count() - 1 }} ? 0 : current + 1"
                                    ...
                                    <div
                                        ...
                                        @foreach ($portfolio->images as $i => $image)
                                            <button @click="current = {{ $i }}"
                                                :class="current === {{ $i }} ? 'border-sbk-red opacity-100' :
                    ...
                    <div class="bg-sbk-gray-light rounded-2xl p-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="font-semibold text-sbk-black text-sm">{{ $portfolio->client->name ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Nama Dokumen</p>
                                <p class="font-semibold text-sbk-black text-sm">{{ $portfolio->document_name }}</p>
                            </div>
                            @if ($portfolio->document_category)
                                <div>
                                    <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Kategori</p>
                                    <p class="font-semibold text-sbk-black text-sm">{{ $portfolio->document_category }}
                                    </p>
                                </div>
                            @endif
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Periode</p>
                                <p class="font-semibold text-sbk-black text-sm">{{ $portfolio->period }}</p>
                            </div>
                            @if ($portfolio->images->count())
                                <div>
                                    <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Dokumentasi</p>
                                    <p class="font-semibold text-sbk-black text-sm">{{ $portfolio->images->count() }} foto
                                    </p>
                                </div>
                            @endif
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-3">
                                @foreach ($related as $r)
                                    <a href="{{ route('portfolio.show', $r->id) }}"
                                  ...
                                  hover:border-sbk-red/20 hover:shadow-soft transition-all duration-200 group">
                                        ...
                                        <div>
                                          ...
                                          group-hover:text-sbk-red transition-colors">
                                            ...
                                            <p class="text-gray-400 text-xs mt-1">{{ $r->period }}</p>
                                        </div>
                                    </a>
                                @endforeach
                            </div>
