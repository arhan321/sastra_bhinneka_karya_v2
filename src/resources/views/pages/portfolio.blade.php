...
@section('content')
    ...
    <section class="bg-sbk-black py-32 relative overflow-hidden">
        <div class="absolute inset-0 z-0">
            <img src="{{ asset('images/construction-site.jpg') }}" class="w-full h-full object-cover"
        ...
        <div class="container-sbk">
            <div class="flex items-center gap-2 overflow-x-auto py-4 scrollbar-hide" x-data="{ active: 'all' }">
                ...
                @foreach ($clients as $client)
                    <button @click="active = '{{ $client->id }}'"
                        :class="active === '{{ $client->id }}' ? 'bg-sbk-red text-white' :
                        ...
                        onclick="filterPortfolio('{{ $client->id }}')">
                        {{ Str::limit($client->name, 30) }}
                    </button>
                @endforeach
            </div>
        </div>
    ...
    <section class="section-padding bg-sbk-gray-light">
        <div class="container-sbk">
            ...
            @forelse($grouped as $clientId => $items)
                @php $clientName = $items->first()->client->name ?? 'Klien' @endphp

                <div class="portfolio-group mb-16" data-client="{{ $clientId }}">
                    ...
                    <div class="flex items-center gap-4 mb-8" data-reveal="fade-right">
                        <div class="w-10 h-10 bg-sbk-red rounded-xl flex items-center justify-center flex-shrink-0">
                            <span class="text-white font-black text-sm">{{ strtoupper(substr($clientName, 3, 1)) }}</span>
                        </div>
                        <div>
                            <p class="text-xs text-sbk-red font-bold uppercase tracking-widest">Dokumentasi Kegiatan</p>
                            <h3 class="font-heading font-black text-sbk-black text-xl">{{ $clientName }}</h3>
                        </div>
                        <div class="flex-1 h-px bg-gray-200 ml-4"></div>
                        <span class="text-xs text-gray-400 font-medium flex-shrink-0">{{ $items->count() }} dokumen</span>
                    </div>

                    {{-- Portfolio Cards --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($items as $i => $portfolio)
                            <a href="{{ route('portfolio.show', $portfolio->id) }}" data-reveal="fade-up"
                                data-delay="{{ ($i % 3) * 100 }}"
